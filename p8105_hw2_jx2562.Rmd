...
There are `r length(only_baseline)` participants only appears in the baseline dataset. They are # `r only_baseline` participants. `r length(only_biomarker)` participants only appears in the amyloid dataset. 
...
*   Their average age at the onset of MCI is `r round(mean(MCI_df$age_at_onset), 2)`, ranging from `r min(MCI_df$age_at_onset)` to `r max(MCI_df$age_at_onset)` .
